...
public class PDF {
  ...
  public int TamanioPDF()
       ...
       public String bytes()
    {
        ...
        tam= archivo.getChannel().size() + " Bytes";
       archivo.close();
            }
          catch (FileNotFoundException ex) {
            Logger.getLogger(Main.class.getName()).log(Level.SEVERE, null, ex);
        ...
        try {
            ...
            while((cadena = archivo.readLine()) != null){
             ...
             {               
                 ...
                 this.ref = Integer.parseInt(cadena);
                            break;
             }
             ...
             for(int j=1; j<a; j++)
             {
                 ...
                 if("I".equals(buscador))
                 {
                     String[] auxPartir = partir[j].split(" ");
                     auxRef = Integer.parseInt(auxPartir[1]);
                     buscador = "";
                 }
                 else if ("R".equals(buscador)) {
                      String[] auxPartir = partir[j].split(" ");
                     r = Integer.parseInt(auxPartir[1]);
                     buscador="";
                 }
                 else{
                     buscador="";
            ...
            if(aux != auxRef + 1)
            {
                ...
                {
                    auxRef = aux;
                    nom = "No se encontro ninguna referencia ";
                }
                aux = auxRef+1;
            }       
            ...
            int contador = 0;
            archivo.seek(this.ref);
            if(!"No se encontro ninguna referencia ".equals(nom))
            {
            while((cadena = archivo.readLine()) != null){
               ...
               {
                   archivo.read(meta);
                   refM = Integer.parseInt(new String(meta));
               archivo.seek(refM);
    ...
    private String getResources(String info)
    {
        ...
        while((cadena = archivo.readLine()) != null){
            if(contador == aux +1)
            {
                ...
                referencia = Integer.parseInt(new String(meta));
                archivo.seek(referencia);
                while(!"end".equals(cadena = archivo.readLine())){
                    info+=cadena;
                }
            }
            contador++;
